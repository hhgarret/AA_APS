...
class Gateway:
    ...
    def parse_line(self, line):
        ...
        if decode[0:4] == "BLE:":
            ...
            if split[1] == "Connected":
                ...
                #FOR FLSAHING ONLY
                time.sleep(3)
                self.master.master.start_ncpa_mode() # gatway->gatewaywindow->controlwindow.start_ncpa_mode()
                # time.sleep(5)
                # self.master.master.nib_read()
            ...
            nibreadlogfile.write(f"Read GPS SOH from {self.curhex}: {decode}\n")
            nibreadlogfile.close()
    ...
    def start_ncpa_mode(self, command):
        for node in self.nodes:
            ...
            time.sleep(1.5) # TODO: find better way to do this? After ACK, maybe?
    ...
    def nib_write_silent(self):
        nodeid = 99 # any node that is out of range (0-25)
        numseconds = pps_to_wait
        cmd = f"NIBWS{nodeid:04d}{numseconds:1d}00000000000000000000000000".encode()
...
class ControlWindow(tk.Tk):
    def __init__(self):
        ...
        self.schedule_all_repeat_button = self.create_button(self.frame2, "Schedule all repeat x10", self.schedule_all_repeat)
    ...
    def schedule_trial_wrapper(self):
        trial_thread = threading.Thread(target=self.schedule_trial)
        trial_thread.start()

    def schedule_trial(self):
        self.schedule_trial_button.configure(bg = 'green')
        self.update()
        numnodes = len(global_nodes)
        nibreadlogfile = open("nibreadlogfile.txt", "a")
        nibreadlogfile.write("Starting a new trial\n")
        nibreadlogfile.close()
        # check status of nodes
        # self.read_soh() # goes through nodes 1 by 1 and reads SOH
        # for i in range(4*numnodes + 1):
        #     time.sleep(1)
        #     self.update()
        # actually go about the trial
        for node in global_nodes:
            nibreadlogfile = open("nibreadlogfile.txt", "a")
            nibreadlogfile.write(f"Scheduling a new node {node}\n")
            nibreadlogfile.close()
            for gateway in self.gateways:
    ...
    def schedule_all_repeat_wrapper(self):
        repeat_thread = threading.Thread(target=self.schedule_all_repeat)
        repeat_thread.start()

    def schedule_all_repeat(self):
        self.schedule_all_repeat_button.configure(bg = 'green')
        self.update()
        for i in range(10): # repeat 10 times
            for gateway in self.gateways: # schedule all nodes to play
                gateway.schedule_node_all()
            self.update()
            for i in range(11): # wait for all nodes to play
                time.sleep(1)
                self.update()
            self.nib_read() # make sure files are written
            for i in range(2): # wait for all nib reads to return
                time.sleep(1)
                self.update()
        self.schedule_all_repeat_button.configure(bg = 'white')
        self.update()
